...
h5py_folder = '/home/karlzipser/Desktop/all_aruco_ready/full_raised/h5py'
...
for r in runs:
	try:
		...
		if 'cmd_motor' in O:
			...
			cmd_motor_ts = L[ts][:]
		mask,cmd_mask = get_data_mask(state=state_,motor=motor_,cmd_motor=cmd_motor_,cmd_use_states=[3,5,6,7],human_use_states=[1,2],min_motor=51,min_cmd_motor=0,original_cmd_motor_ts=cmd_motor_ts,ts=L[ts][:])
		m = mask + cmd_mask
		m = m[:len(P['ax'][:])]
