...
if False:
	rp_computer_name = 'Mr_'
	existing_bag_files = sgg(opjm('rosbags/*.bag'))
	folder_name = existing_bag_files[0].replace('bair_car',rp_computer_name)
	folder_name = '_'.join(folder_name.split('_')[:-1])
	unix('mkdir '+folder_name)
	for e in existing_bag_files:
		unix(d2s('mv',e,folder_name))
	active_bag_files = sgg(opjm('rosbags/*.bag.active'))
	for a in active_bag_files:
		unix(d2s('rm',a))
