...
while P['cmd/initalize net.'] == False:
    reminder_timer.message("Please set P['cmd/initalize net.'] to True...")
    time.sleep(0.1)

if P['cmd/initalize net.'] == True:
...
while P['cmd/start training,'] == False:
    reminder_timer.message("Please set P['cmd/start training,'] to True...")

while P['ABORT'] == False:
    
    if P['cmd/pause training,'] == True:
        reminder_timer.message("Please set P['cmd/pause training,'] to False...")
        time.sleep(0.1)
        continue


    for i in range(batch_size):
        D = get_input_output_data(L,int(I['sig_sorted'][-np.random.randint(P['dat/sig sorted value,']),0]),P)
        IO = {}
