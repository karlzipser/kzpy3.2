...
class SqueezeNet(nn.Module):
    def __init__(
        ...
        self.L['pre_metadata_features'] = nn.Sequential(
            nn.Conv2d(NUM_INPUT_CHANNELS, 64, kernel_size=3, stride=2),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=3, stride=2, ceil_mode=True),
            Fire(64, 16, 64, 64),            
            )
        self.L['post_metadata_features'] = nn.Sequential(
        ...
        self.L['final_output'] = nn.Sequential(
            nn.Dropout(p=0.5),
            final_conv,
    ...
    def forward(self,input_data,meta_data,target_data):
        ...
        self.A['pre_metadata_features'] = self.L['pre_metadata_features'](self.A['camera_input'])
        if type(meta_data) == type(None):
        ...
        self.A['post_metadata_features'] = self.L['post_metadata_features'](self.A['pre_metadata_features_metadata'])
        self.A['final_output'] = self.L['final_output'](self.A['post_metadata_features'])
