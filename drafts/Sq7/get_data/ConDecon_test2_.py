...
n = 5
input_data_plus = zeros((3,2*n+41,2*n+23))#+0.5
target_data_plus = zeros((3,2*n+41,2*n+23))+0.5
...
def get_data_function(P):
    ...
    targets = False
    for i in [2]:#,1,0]:
        D = get_data_function_0(P,ctr,r,flip)
        if type(inputs) == type(False):
            inputs = na(D['input'])
            targets = na(D['target'])
        else:
            pass
            #inputs = np.concatenate((inputs,na(D['input'])),0)
            #targets = np.concatenate((targets,na(D['target'])),0)
        ctr -= 1
        #print ctr
...
def get_data_function_0(P,ctr,r,flip):
    ...
    target_data_plus[:,n:n+41,n:n+23] = target_data
    input_data_plus[:,n:n+41,n:n+23] = input_data
