    ...
    while P['index'] < len(U['ts']):
        ...
        if True:
            ...
            try:
                ...
                #print shape(pts),type(pts),shape(rpoints),type(rpoints)
                pts += list(rpoints)
                if len(pts)>P['num points']:
                    pts = pts[-P['num points']:]
                pts = rotatePolygon(pts,d_heading)
                pts.append(a)
                pts = list(na(pts)-pts[-1])
                indx += 1
                clf();pts_plot(na(pts));plt_square();xysqlim(P['l'])
                plot(0,0,'b.')
