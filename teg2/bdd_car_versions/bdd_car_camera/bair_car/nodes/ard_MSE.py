...
def mse_write_publish(M,Arduinos,steer_pwm,motor_pwm):
    ...
    write_str = d2n( '(', int(steer_pwm), ',', int(motor_pwm+10000), int(motor_pwm+20000),')')
...
def serial_data_to_messages(Arduinos,M):
    ...
    try:
        ...
        return False
    if len(mse_input) == 6 and mse_input[0] == 'mse':
        lock.acquire()
        M['button_pwm_lst'].append(mse_input[1])
        M['steer_pwm_lst'].append(mse_input[2])
        M['motor_pwm_lst'].append(mse_input[3])
        M['camera_pwm_lst'].append(mse_input[4])
        M['encoder_lst'].append(mse_input[5])
