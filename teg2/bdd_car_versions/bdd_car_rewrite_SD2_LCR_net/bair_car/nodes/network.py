...
def run_model(input, metadata):
    ...
    torch_motor = 100 * output[0][10+3].data[0]
    torch_steer = 100 * output[0][0+3].data[0]
...
one_matrixv = torch.FloatTensor(1, 23, 41).fill_(1)
...
def format_metadata(raw_metadata):
    ...
    for target_statev in [1,2,3]:
        ...
        for i in range(10):
            metadatav = torch.cat((use_matrixv, metadatav), 0)
            mode_ctrv += 1
