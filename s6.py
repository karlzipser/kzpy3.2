...
CA()
run_names = sggo('/media/karlzipser/rosbags/tu_15to16Nov2018/locations/local/left_direct_stop/h5py/*')
...
for r in run_names:
    ...
    D=h5r('/home/karlzipser/Desktop/Depth_images_15_16Nov2018/'+run_name+'.Depth_image.log.resize.flip.with_left_ts.h5py' )    

    O=h5r('/media/karlzipser/rosbags/tu_15to16Nov2018/locations/local/left_direct_stop/h5py/'+run_name+'/original_timestamp_data.h5py')


    timer = Timer(60)
    timer2 = Timer(1)#/30.)
    cg('here')
    figure(0);clf();plot(D['left_to_lidar_index'],'b,')
    
    for n in range(0,len(D['left_to_lidar_index']),len(D['left_to_lidar_index'])/20):
        #n = np.random.randint()
        l = D['left_to_lidar_index'][n]
        if l < 1:
            cr(n,l,len(D['resized']))
            continue
        else:
            cg(n,l,len(D['resized']))
        figure(0);plot(n,l,'r.');spause()
        mi(O['left_image']['vals'][n],1) 
        mi(D['resized'][l],2)
        spause()
        while timer2.check() == False:
            time.sleep(1/300.)
        timer2.reset()
        raw_enter(run_name+'  ')
