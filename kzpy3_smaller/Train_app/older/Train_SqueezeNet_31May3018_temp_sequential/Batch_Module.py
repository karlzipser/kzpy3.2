...
if HAVE_GPU:
	...
	one_matrix = torch.FloatTensor(1, 1, 23, 41).fill_(1)
...
def Batch(the_network=None):
	...
	if HAVE_GPU:
		...
		D['camera_data'] = torch.FloatTensor()
		D['metadata'] = torch.FloatTensor()
		D['target_data'] = torch.FloatTensor()
		...
		while ctr < D['batch_size']:
			if True:#try:
				...
				if HAVE_GPU:
					...
					steerv = torch.from_numpy(sv).float() / 99.
					motorv = torch.from_numpy(mv).float() / 99.
	...
	def _function_forward():
		...
		if HAVE_GPU:
			...
			D['outputs'] = D['network']['net'](torch.autograd.Variable(D['camera_data']), torch.autograd.Variable(D['metadata']))
	...
	def _function_backward():
		try:
			...
			#spd2s(D['loss'],D['loss'].detach(),D['loss'].detach().numpy())
			if HAVE_GPU:
				P['LOSS_LIST'].append(D['loss'].data.cpu().numpy()[:].mean())
			else:
				d = D['loss'].detach().numpy()
				P['LOSS_LIST'].append(d)
				#print len(P['LOSS_LIST'])
		...
		if P['print_timer'].check():
			for i in [0]:#range(P['BATCH_SIZE']):
				if True:#HAVE_GPU:
					...
					ov = D['outputs'][i].data.numpy()
					mv = D['metadata'][i].numpy()
					tv = D['target_data'][i].numpy()
					av = D['camera_data'][i][:].numpy()

				if HAVE_GPU: 
					print('Loss:',dp(D['loss'].data.cpu().numpy()[0],5))
				else:
					print D['loss'],type(D['loss'])
					#print(D['loss'].data.cpu.numpy())
					#print(D['loss'].data.cpu().numpy())
				...
				figure('steer '+P['start time'],figsize=(3,4))
				clf()
				plt.title(d2s(i))
				...
				plt.xlabel(d2s(bm))
				if False:
					figure('metadata '+P['start time'],figsize=(1,1));clf()
					plot(mv[-10:,0,0],'r.-')
					plt.title(d2s(bm,i))
				spause()
				P['print_timer'].reset()
