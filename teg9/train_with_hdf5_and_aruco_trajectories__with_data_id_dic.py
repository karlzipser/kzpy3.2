...
def get_data_considering_high_low_steer_and_valid_trajectory_timestamp__with_data_id(d):
...
if 'data_id_dic' not in locals():
	data_id_dic = lo(opjD('data_id_dic'),True)
all_indicies = data_id_dic.keys()


while True:
	random.shuffle(all_indicies)
	for i in range(len(all_indicies)):
		for b in range(Solver.batch_size):
			data = None
			while data == None:
				data = get_data_considering_high_low_steer_and_valid_trajectory_timestamp__with_data_id({'all_index':all_indicies[i]})
			assert(data != None)
