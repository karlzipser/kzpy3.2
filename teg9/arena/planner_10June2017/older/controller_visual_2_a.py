...
for our_car in ['Mr_Yellow','Mr_Black','Mr_Silver','Mr_Orange','Mr_Blue']:#['Mr_Yellow','Mr_Orange','Mr_Blue']:#
	...
	for run_name in cars[our_car]['runs'].keys():
		...
		try:
			...
			for the_arena_type in ['Furtive_Arena_Potential_Field','Direct_Arena_Potential_Field',
				...
				output_data[run_name][mode]['plan_velocity'] = []
				output_data[run_name][mode]['current_direction_max'] = []
				...
				for t in arange(T0,Tn,1/30.):
					...
					if len(xy_our) > 0:
						...
						if cars[our_car]['state_info']['heading'] != None:
							...
							current_direction = array(steers[-(30*10):]).mean()
							if current_direction > 49:
								current_direction_max = 99
							else:
								current_direction_max = 0
							dist = (1/m[3]+1/m[4])/2.0
							plan_vel = 1
							if dist < 2:
								if cars[our_car]['state_info']['relative_heading'] < 45:
									#steer = current_direction_max

									print(d2s('crash danger!',cars[our_car]['state_info']['relative_heading']))
									plan_vel = 0.75
							if dist < 1:
								if cars[our_car]['state_info']['relative_heading'] < 45:
									#steer = current_direction_max
									print('crash danger! Stop!')
									plan_vel = 0
							steers.append(steer)
							...
							output_data[run_name][mode]['plan_velocity'].append(plan_vel)
							...
							output_data[run_name][mode]['current_direction_max'].append(current_direction_max)
							...
							if GRAPHICS:
								...
								if type(img_left_previous) != bool:
									...
									vel = (vel+vel_prev)/2.0
									vel_prev = vel
								img = img_left.copy()
								k = animate.prepare_and_show_or_return_frame(img=img,steer=steer,motor=49+20*vel,state=6,delay=1,scale=2,color_mode=cv2.COLOR_RGB2BGR,window_title='network')
								img_left_previous = img_left
								img_right_previous = img_right
								print(dp(vel,1),dp(plan_vel,1))
								if k == ord('q'):
									break
		...
		del cars[our_car]['runs'][run_name]
