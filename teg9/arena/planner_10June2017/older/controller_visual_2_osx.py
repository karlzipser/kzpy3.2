...
if USE_CAFFE:
	...
	#solver.net.copy_from(most_recent_file_in_folder(opjh('caffe_models/z2_color_aruco_potential_May2017'),'650'))
	solver.net.copy_from('/Users/karlzipser/caffe_models/z2_color_aruco_potential_May2017/z2_color_aruco_iter_3000000.caffemodel')
				...
				for t in arange(T0+210,Tn,1/30.):
					...
					if len(xy_our) > 0:
						...
						if cars[our_car]['state_info']['heading'] != None:
							...
							solver.net.blobs['target_markers'].data[-1,:] = m
					if GRAPHICS2:
						"""
						if ctr_q > 3:
							figure(9)
