...
pythonpaths(['kzpy3','kzpy3/teg9','kzpy3/pytorch3'])
from vis2 import *
...
def save_net(d):
    net = d['net']
    weights_folder_path = d['weights_folder_path']
    ...
    if 'now' in d:
        ...
        now = False
    True
    if P.save_net_timer.check() or now:
        weights_file_name = 'save_file' +time_str()
        torch.save(net.state_dict(), opjh(weights_folder_path,weights_file_name+'.weights'))
        ...
        torch.save(weights, opjh(weights_folder_path,weights_file_name+'.infer'))
