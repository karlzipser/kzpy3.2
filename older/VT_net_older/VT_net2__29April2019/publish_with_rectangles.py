...
import cv_bridge
from kzpy3.VT_net2__1June2019.p12 import get_images
#bridge = cv_bridge.CvBridge()
exec(identify_file_str)
_ = default_values._

Defaults = {
    'pub_predictions':False,
    'step':False,
...
rospy.Subscriber('/ldr_img',Image,image_callback,queue_size = 1)
...
rate_timer = Timer(1/10000.)#Timer(1/30.)
rot = 4.
rect_on = Timer(rot)

left_list = []
ldr_list = []
raw_enter(str(len(U['ts'])))
if __name__ == '__main__':
    ...
    if True:#try:
        while _['index'] < len(U['ts']) and not _['ABORT']:
            ...
            if rate_timer.check():
                ...
                """
                if rect_on.time() < rot/2.:
                    try:
                        left_image,right_image = get_images(left_image,right_image)
                ...
                mci(image_list[-1],title='ldr_img...',scale=4.,delay=1)
                left_list.append(left_image.copy())
                ldr_list.append(image_list[-1].copy())
                print len(left_list)
                mci(left_list[-1],title='left_image',delay=1)
    ...
    out = h5w(opjD('out.h5py'))
    out.create_dataset('left',data=na(left_list))
    out.create_dataset('ldr',data=na(ldr_list))
    out.close()
    """
