...
P['MODEL'] = 'SqueezeNet'
P['RESUME'] = False
P['bair_car_data_path'] = opjD('bair_car_data_Main_Dataset')
if P['RESUME']:
    P['weights_file_path'] = 'xxx'
P['ignore'] = ['reject_run','left','out1_in2']#,'Smyth','racing','local','Tilden','campus']
P['require_one'] = []
P['use_states'] = [1,3,5,6,7]
P['rate_timer_interval'] = 5.
P['print_timer'] = Timer(5)
P['save_timer'] = Timer(60*15)
...
rate_timer = Timer(P['rate_timer_interval'])
rate_ctr = 0
...
def get_data_considering_high_low_steer():
    ...
    data = get_data_with_hdf5.get_data(run_code,seg_num,offset,net.N_STEPS,offset+0,net.N_FRAMES,ignore=P['ignore'],require_one=P['require_one'],use_states=P['use_states'])
...
while True:
    ...
    for b in range(P['BATCH_SIZE']):
        ...
        rate_ctr += 1
        if rate_timer.check():
            print(d2s('rate =',dp(rate_ctr/P['rate_timer_interval'],2),'Hz'))
            rate_timer.reset()
            rate_ctr = 0
