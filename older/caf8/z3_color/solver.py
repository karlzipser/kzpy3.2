...
train_val_lst += [
	...
	protos.dummy('steer_motor_target_data',(batch_size,60)),
	protos.dummy('metadata',(batch_size,16,14,26)),
	protos.dummy('ZED_data_pool2',(batch_size,18,94,168)),
	...
	protos.ip("ip2","ip1",60,"xavier",std=0),
...
def put_data_into_model(data,solver,b=0):
	...
	solver.net.blobs['steer_motor_target_data'].data[b,:30] = data['steer'][-30:]/99.
	solver.net.blobs['steer_motor_target_data'].data[b,30:] = data['motor'][-30:]/99.
	#
	##########################################################
