...
class Area_Visualizer(object):
    ...
    def visualize_markers_center_line(self, markers, filename,cv_image):
        ...
        if False:
            mi(cv2.cvtColor(vis[:550,:],cv2.COLOR_BGR2RGB));pause(0.01)
        if True:
            for q in range(2):
                imsave(opjD('aruco_markers_frames',d2n(ctr,'.png')),cv2.cvtColor(vis[:550,:],cv2.COLOR_BGR2RGB))
                print(ctr)
                ctr += 1
...
if __name__ == "__main__":
    ...
    bag_num_0 = int(sys.argv[2])
    bag_num_1 = int(sys.argv[3])
    bag_files = sgg(opj(bag_folder_path,'*.bag'))
    #raw_input((bag_num_0,bag_num_1))
    for i in range(bag_num_0,bag_num_1):

        bagfile_path = bag_files[i]

        print(i,bagfile_path)
        ...
        try:
            while(True):
                ...
                #mi(cv_image);pause(0.001)
                print(i,bag_num_0,bag_num_1)
